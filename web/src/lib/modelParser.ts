...
const APOCALYPSE_SURVIVAL_SEMANTIC_GROUPS: SemanticActionGroup[] = [
  // 1. 水源/干粮/物资恩赐与等价交换
  {
    keywords: ['水', '喝水', '口渴', '饼干', '食物', '饿', '干渴', '物资', '压缩饼干', '抗生素', '纯净水'],
    branches: [
      ...
      { title: '将仅剩的半盒消炎胶囊扣下作为长期支配筹码', desc: '锁紧药盒直视苏晓染慌乱的美眸，让她清楚谁才是她生命的绝对主宰【核心拿捏: 掌握救命药品，彻底瓦解其反叛心理】' }
    ]
  },
  // 2. 尸群/悬殊战力/打不过/致命绝境与阵亡读档
  {
    keywords: ['尸群', '打过', '全灭', '尸潮', '打不过', '扑倒', '撕咬', '丧尸', '咬死', '阵亡', '死亡', '濒死', '围攻', '绝境', '死战'],
    branches: [
      { title: '【💀 壮烈战死·读档上一幕】', desc: '承认凡人肉躯无法抗衡庞大尸潮的铁律，撤销盲目蛮干并读档回溯至安全时点重新布局【绝境重来: 及时止损，避免全军覆没】' },
      { title: '声东击西投掷铁罐引开主力尸群逃向侧翼', desc: '抓起脚边的生锈铁罐猛力砸向远处废墟，利用尖锐声响吸引尸群听觉为撤退争取死线【战术脱离: 避免正面硬碰，利用规则智取生机】' },
      { title: '拉起苏晓染拼死撤回地下安全屋重锁防爆门', desc: '放弃毫无胜算的搏杀，一把拽起发呆瘫软的苏晓染撞入安全门内瞬间插死锁芯【止损固守: 舍弃外围领地，退守坚固掩体】' },
      { title: '引燃残存煤油瓶构筑火墙阻断追击', desc: '点燃简易燃烧瓶砸向狭窄过道，利用熊熊烈火阻隔疯狂追击的感染者群【断尾求生: 消耗稀缺燃料，换取宝贵的喘息时机】' },
      { title: '【苏晓染舍命救援·断臂逃生】', desc: '在被尸群扑倒的濒死瞬间，苏晓染泣血挥动铁棒砸开丧尸，合力滚入通风管道【惨烈生还: 逃出生天但承受严重创伤，心性剧烈蜕变】' }
    ]
  },
  // 3. 枪械/防爆门/检疫排查与威权掌控
  {
    keywords: ['枪', '门', '防爆门', '安全屋', '防空洞', '酸雨', '辐射', '危险', '检疫', '感染', '排查'],
    branches: [
      ...
      { title: '在安全屋门口布置拉线绊雷与空罐警报铃', desc: '利用废旧导线和空罐头在门口拉起双重绊线，防止夜间突发尸潮偷袭【阵地构筑: 提升防御等级，获得夜间安全感知】' }
    ]
  },
  // 4. 肉体臣服/校花阶级瓦解/领地初夜特权
  {
    keywords: ['跪', '屈服', '衣服', '扯破', '锁骨', '床上', '占有', '臣服', '身体', '求饶', '苏晓染'],
    branches: [
      ...
      { title: '令其赤足坐在床沿为自己擦拭靴底血污', desc: '践踏昔日高不可攀医学校花的最后骄傲，使其习惯于贴身服侍【阶级反转: 确立绝对尊卑，彻底固化主仆身份】' }
    ]
  },
  // 5. 搜刮潜行/撬锁破门/废墟探索
  {
    keywords: ['搜寻', '搜刮', '撬锁', '保险柜', '货架', '便利店', '下水道', '手电', '黑暗', '探索', '潜行'],
    branches: [
      { title: '压低手电光束搜寻角落蒙尘的应急医药箱', desc: '用指缝挡住手电微光，蹑手蹑脚翻查药房货架底层可能残留的止痛片【战术搜刮: 规避光影外泄，寻找救命医用耗材】' },
      { title: '使用生锈发卡小心翼翼尝试撬动铁皮柜暗锁', desc: '屏住呼吸转动锁芯，提防机械回响惊动隔壁游荡的听觉变异体【精密开锁: 探寻前人遗留机密，承担声响暴露风险】' },
      { title: '沿墙根阴影俯身潜行避开开阔的主干道', desc: '踩着碎石间的棉质杂物压低重心推进，最大程度降低被变异生物目击概率【隐蔽潜入: 保持隐匿状态，绘制安全撤退路线】' },
      { title: '攀上通风管道俯瞰下方游荡尸群动向', desc: '顺着生锈铁架爬入天花板管网，从高空侦查整个街区的尸群巡逻盲区【高维视野: 避开地面杀戮场，获取全局情报优势】' }
    ]
  },
  // 6. 受伤急救/止血包扎/感染抗原排查
  {
    keywords: ['伤口', '血', '包扎', '纱布', '止血', '缝合', '感染', '高烧', '剧痛', '消毒', '酒精'],
    branches: [
      { title: '取出随身医用酒精为伤处浇灌紧急消毒', desc: '强忍烈火灼烧般的剧痛清理受创皮肉，杜绝末世致命杂菌与尸毒滋生【铁血自救: 承受高额痛楚，压制感染爆发风险】' },
      { title: '命令懂医术的苏晓染跪在面前协助清创包扎', desc: '发挥苏晓染医学院专长，在肢体亲密接触中令其为自己缝合止血【专业护理: 缓解创伤恶化，加深二人命运捆绑】' },
      { title: '用火烤红军刀刀尖对创口进行焦化止血', desc: '在缺乏缝合线的情况下采取战地极端焦灼法封闭喷血创口【极限军医: 争分夺秒锁住体征，避免失血过多昏厥】' },
      { title: '给苏晓染注射广谱抗生素并贴身监测其体温', desc: '搂住发抖受惊的苏晓染，用掌心抚摸其额头严密监测其是否发生感染异变【温存守护: 守住核心同伴，避免阵营内部生化暴雷】' }
    ]
  }
];
...
const PERFECT_GIRL_SEMANTIC_GROUPS: SemanticActionGroup[] = [
  {
    keywords: ['怀月', '副主席', '学霸', '栀子花', '杏眼', '泪水', '崩溃', '长椅', '公园', '冷雨'],
    branches: [
      { title: '递上面巾纸轻声询问江怀月的心事', desc: '放轻脚步递上纸巾，温和倾听她在沉重期望下的委屈与疲惫【纯爱救赎: 消除心理戒备，建立最初的信任】' },
      { title: '脱下外套披在单薄颤抖的少女肩头', desc: '默默靠在长椅边为她遮挡夜风，用体温给予无声的安全感【温柔守护: 触动紧绷心防，让她主动寻求依赖】' },
      { title: '捡起地上画满叛逆涂鸦的秘密笔记本', desc: '翻看少女隐藏在模范生面具下的真实渴望，点破她对自由的向往【看穿面具: 触碰灵魂深处，撕开完美人设伪装】' }
    ]
  },
  {
    keywords: ['纯爱', '救赎', '治愈', '拥抱', '牵手', '倾听', '心愿', '眼泪'],
    branches: [
      { title: '牵起她冰凉纤细的双手轻声许诺', desc: '向她保证今夜无论说什么都绝不会被第三人知晓，做她唯一的避风港【心灵契约: 成为唯一托底者，抚平家庭创伤】' },
      { title: '拉着她到全糖奶茶店把想喝的通通买下', desc: '鼓励她放下对卡路里和规训的负罪感，尽情享受甜蜜微甜的放纵【破戒启蒙: 打破苛刻自律，唤醒少女的纯真快乐】' },
      { title: '轻柔拭去她长睫毛上挂着的晶莹泪珠', desc: '俯身温柔凝视她清澈无辜的杏眼，抚平她对失去第一名的极度恐惧【温存抚慰: 肯定其存在价值，救赎濒临碎裂的自尊】' }
    ]
  },
  {
    keywords: ['堕落', '放纵', '香烟', '啤酒', '控制', '依赖', '夜游', '摇滚'],
    branches: [
      { title: '拉开冰镇啤酒递到她面前引诱放纵', desc: '带着坏笑引诱好学生破戒尝鲜，欣赏她呛咳脸红后的羞怯反叛【放纵诱导: 击碎道德规训，开启滑向堕落的第一步】' },
      { title: '挑起她泛红的小脸逼她直视叛逆的快感', desc: '告诉她好学生并不需要当一辈子提线木偶，享受打破禁忌的狂乱【心智掌控: 颠覆既定认知，让其对离经叛道欲罢不能】' },
      { title: '命令江怀月在深夜空旷街头大声宣泄呐喊', desc: '带她撕开所有斯文礼貌的面具，在无人长街上尽情尖叫与叛逆狂欢【野性释放: 彻底解封深层自我，建立对玩家的绝对依附】' }
    ]
  }
];

// 🚪 《调皮的女儿用自己的身体堵门》主题分支库
const DAUGHTER_DOOR_BLOCK_SEMANTIC_GROUPS: SemanticActionGroup[] = [
  {
    keywords: ['堵门', '门把', '赤足', '光脚', '小腿', '睡裙', '横跨', '下摆'],
    branches: [
      { title: '伸手握住女儿踩在门把上的粉嫩脚踝', desc: '指腹轻抚她微凉细腻的脚背与脚踝，半是责备半是宠溺地往下拉【肢体推拉: 试探私密触碰，让女儿娇蛮中泛起羞意】' },
      { title: '反手将堵门的林可可按在厚重门板上', desc: '顺势逼近将她圈在怀中，低下头贴近她滚烫的小耳朵沉声低语【壁咚压制: 展现男性力量，打破父女原本的安全距离】' },
      { title: '揉乱她蓬松的发顶苦笑保证晚上早归', desc: '无奈地抚摸女儿柔软发丝，承诺下班带她吃最喜欢的大餐化解危机【温情安抚: 顾全大局，维持慈父形象化解迟到危机】' }
    ]
  },
  {
    keywords: ['吃醋', '争宠', '妈妈', '搂搂抱抱', '亲热', '亲亲', '抱抱', '委屈'],
    branches: [
      { title: '趁厨房动静迅速低头在可可脸颊轻吻一口', desc: '戒备厨房妻子动静，飞快在她软嫩脸颊上亲一下哄她让开【心惊偷欢: 满足女儿争宠要求，刺激度瞬间拉满】' },
      { title: '板起脸严厉训斥她越来越没大没小要动家法', desc: '以严父威严震慑女儿的肆无忌惮，逼她退后收回任性长腿【父权惩戒: 树立家长权威，压制过于危险的占有欲】' },
      { title: '顺水推舟搂住她柔软纤细的腰肢耳语调戏', desc: '贴着她的耳垂低语：“连妈妈的醋都吃，那爸爸今天就把你也抱个够？”【危险越界: 顺应争宠挑衅，将家庭伦理推向禁忌深渊】' }
    ]
  },
  {
    keywords: ['厨房', '妻子', '早餐', '脚步', '迟到', '全勤', '上班', '门锁'],
    branches: [
      { title: '慌忙朝厨房张望并抬手捂住可可乱嚷的嘴唇', desc: '一手捂住女儿喋喋不休的小嘴，一手将她娇软身子往身侧拉开【险象环生: 阻止妻子察觉，在极度心虚中仓促周旋】' },
      { title: '故意大声回应厨房让林婉清把公文包递来', desc: '借由妻子的脚步声恐吓堵门的女儿，看她慌乱中是否敢继续放肆【借刀杀人: 利用母亲存在施压，逼可可自行松开门把】' },
      { title: '横抱起乱蹬双腿的可可快步送回她自己卧室', desc: '不容分说将赤脚的女儿打横抱起塞回房间床上，迅速返回反锁大门【强行脱困: 果断中断纠缠，确保工作全勤不被扣罚】' }
    ]
  }
];

// 👠 《成年了还把你当孩子的巨乳美母骚姐》主题分支库
const MOTHER_SISTER_BABY_SEMANTIC_GROUPS: SemanticActionGroup[] = [
  {
    keywords: ['姐姐', '清颜', '模特', '自慰', '昨夜', '撞破', '旧t恤', '脸红', '丹凤眼'],
    branches: [
      { title: '餐桌前给耳朵通红的顾清颜推过去草莓果酱', desc: '假装昨夜自己睡得太沉什么都没看见，给窘迫的超模姐姐一个台阶【递台阶: 缓解尴尬对视，暗中欣赏高岭之花的羞态】' },
      { title: '凑近姐姐低语昨夜门缝里的喘息其实很好听', desc: '用最放肆的低语挑破私密秘密，将高傲超模逼到咬唇羞愤的绝境【反客为主: 撕破弟弟身份伪装，掌握姐姐的绝对隐私把柄】' },
      { title: '眼神肆无忌惮流连在姐姐T恤下的九头身曲线', desc: '直勾勾打量她不穿内衣的领口与修长美腿，逼得她局促收拢双腿【目光侵略: 用成年男性的侵略性视线打破家人边界】' }
    ]
  },
  {
    keywords: ['妈妈', '沈韵', '浴巾', '巨乳', '雪乳', '吹头发', '舞蹈', '温柔', '大平层'],
    branches: [
      { title: '接过电吹风走到沙发后替母亲轻柔吹拂湿发', desc: '指尖穿过沈韵湿润温软的紫色长发，目光近距离掠过她单薄浴巾下的H罩杯雪乳【贴身侍奉: 享受熟女母亲的毫无设防，心跳疯狂加速】' },
      { title: '温顺地张开嘴吃下妈妈亲手喂过来的烤吐司', desc: '乖乖配合母亲的投喂习惯，在姐姐异样的目光中享受母亲的宠溺【争宠修罗场: 借母子亲昵刺激羞耻的姐姐，拉扯微妙家庭气氛】' },
      { title: '在母亲弯腰端盘时贴心替她拉了拉下滑的浴巾', desc: '借整理衣物之名指尖若有若无划过熟女人妻的香肩，提醒她注意走光【边界试探: 似正经实暧昧的触碰，在熟女心湖投下涟漪】' }
    ]
  },
  {
    keywords: ['修罗场', '三人', '客厅', '沙发', '夜宵', '同居', '无防备', '更衣'],
    branches: [
      { title: '坐在姐姐与母亲中间若无其事地打开电视', desc: '陷在宽大沙发中感受两具截然不同却极具杀伤力的成熟躯体包围【左拥右抱: 沉浸大平层的私密温柔乡，静观家庭边界融化】' },
      { title: '借口高三毕业成年提出以后大家都该敲门', desc: '一本正经地在餐桌上提出立规矩，逼姐姐和母亲不得不正视自己已是成年雄性【成年宣誓: 终结小男孩时代，逼全家以异性眼光重新审视彼此】' },
      { title: '趁母亲去洗碗在餐桌下轻轻用脚尖蹭了蹭姐姐的脚踝', desc: '在母亲背对的视线死角展开大胆调情，看姐姐如何在惊慌羞耻中强忍叫声【桌下偷欢: 在家庭日常下展开窒息刺激的暗度陈仓】' }
    ]
  }
];
...
export function generateContextualBranches(
  ...
  const isDaughterDoorBlock =
    deckKey === 'deck_daughter_door_block' ||
    deckKey.includes('2168197e') ||
    deckKey.includes('daughter_door_block');
  const isMotherSisterBaby =
    deckKey === 'deck_mother_sister_baby' ||
    deckKey.includes('758e40b4') ||
    deckKey.includes('mother_sister_baby');
  ...
  else if (isDaughterDoorBlock) semanticGroups = DAUGHTER_DOOR_BLOCK_SEMANTIC_GROUPS;
  else if (isMotherSisterBaby) semanticGroups = MOTHER_SISTER_BABY_SEMANTIC_GROUPS;
